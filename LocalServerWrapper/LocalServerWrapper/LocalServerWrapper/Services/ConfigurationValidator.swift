...
private let logger = OSLog(subsystem: "com.localserverwrapper.configmanager", category: "validation")
...
class ConfigurationValidator: ConfigurationValidatorProtocol {
    ...
    func validate(_ config: ServerConfiguration) throws {
        os_log(.debug, log: logger, "Validating configuration: %{public}@", config.name)
        
        // Validate required field: name
        if config.name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            os_log(.error, log: logger, "Validation failed: missing name")
        ...
        os_log(.debug, log: logger, "Validation passed for: %{public}@", config.name)
    }
    ...
    func validateRegexPattern(_ pattern: String) throws {
        do {
            ...
            os_log(.debug, log: logger, "Regex pattern valid: %{public}@", pattern)
        } catch {
            os_log(.error, log: logger, "Invalid regex pattern: %{public}@", pattern)
